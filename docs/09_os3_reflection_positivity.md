...
**Time reflection.** The operator $\Theta: (x_0, \vec{x}) \mapsto (-x_0, \vec{x})$ flips the time coordinate. It is an involution ($\Theta^2 = \mathrm{id}$) and preserves Lebesgue measure.

**Positive-time test functions.** The space $\mathcal{S}_+$ consists of Schwartz functions supported on $\{x : x_0 > 0\}$.

**Reflection positivity inner product.** For the free covariance $C(x,y) = \frac{m}{4\pi^2\|x-y\|} K_1(m\|x-y\|)$:
$$\langle \Theta f, f \rangle_C := \int_{\mathbb{R}^4} \int_{\mathbb{R}^4} \overline{f(\Theta x)}\  C(x,y)\  f(y)\  dx\  dy$$

**OS3 axiom (Glimm-Jaffe form).** For any positive-time test functions $f_1, \ldots, f_n \in \mathcal{S}_+$ and real coefficients $c_1, \ldots, c_n \in \mathbb{R}$:
$$\sum_{i,j} c_i c_j \  \mathrm{Re}\  Z[f_i - \Theta f_j] \geq 0$$
where $Z[f] = \exp(-\tfrac{1}{2}\langle f, Cf \rangle)$ is the GFF generating functional.
...
|-------------|-------------|
| [`heatKernel_eq_gaussianFT`](../OSforGFF/OS3_MixedRepInfra.lean#L192) | $H_t(z) = \mathcal{F}[e^{-t\|k\|^2}](z)$ |
| [`schwinger_bilinear_integrable`](../OSforGFF/OS3_MixedRep.lean#L883) | $\int_0^\infty e^{-sm^2} \|\langle f, H_s g\rangle\|\ ds < \infty$ |
...
| [`heatKernel_bilinear_fourier_form`](../OSforGFF/OS3_MixedRep.lean#L259) | $\langle f, H_s g\rangle = \int e^{-s\|k\|^2}\hat{f}(-k)\hat{g}(k)\ dk$ |
...
| [`fubini_s_ksp_swap`](../OSforGFF/OS3_MixedRepInfra.lean#L2849) | Fubini swap: proper-time $s$ vs spatial momentum $\vec{k}$ |
| [`fubini_s_xy_swap`](../OSforGFF/OS3_MixedRepInfra.lean#L3065) | Fubini swap: proper-time $s$ vs spacetime $(x,y)$ |
| [`fubini_ksp_xy_swap`](../OSforGFF/OS3_MixedRepInfra.lean#L3548) | Fubini swap: spatial momentum $\vec{k}$ vs spacetime $(x,y)$ |

### Covariance RP direct proof (`OS3_CovarianceRP.lean`)

| Declaration | Description |
|-------------|-------------|
| [`QFT.RPProof.rpInnerProduct`](../OSforGFF/OS3_CovarianceRP.lean#L116) | RP inner product: $\langle \Theta f, f\rangle_C$ via Bessel kernel |
| [`QFT.RPProof.weightedLaplaceFourier`](../OSforGFF/OS3_CovarianceRP.lean#L111) | $F_\omega(\vec{k}) = \int f(x)\ e^{-\omega x_0}e^{-i\vec{k}\cdot\vec{x}}\ dx$ |
| [`QFT.RPProof.mixed_representation`](../OSforGFF/OS3_CovarianceRP.lean#L149) | Mixed representation of the RP inner product |
| [`QFT.RPProof.factorization_to_squared_norm_direct`](../OSforGFF/OS3_CovarianceRP.lean#L257) | Positive-time factorization into $\lvert F_\omega\rvert^2$ |
| [`QFT.RPProof.rp_equals_squared_norm_integral`](../OSforGFF/OS3_CovarianceRP.lean#L351) | $\langle\Theta f, f\rangle_C = \frac{1}{2(2\pi)^3}\int \frac{\lvert F_\omega(\vec{k})\rvert^2}{\omega}\ d\vec{k}$ |
| [`QFT.RPProof.freeCovariance_reflection_positive_direct`](../OSforGFF/OS3_CovarianceRP.lean#L376) | $\mathrm{Re}\langle \Theta f, f\rangle_C \geq 0$ |

### Bridge and real formulation (`OS3_CovarianceRP.lean`)

| Declaration | Description |
|-------------|-------------|
| [`QFT.rpInnerProduct`](../OSforGFF/OS3_CovarianceRP.lean#L79) | RP inner product in QFT namespace |
| [`QFT.rpInnerProduct_eq_rpProof`](../OSforGFF/OS3_CovarianceRP.lean#L410) | Bridge: QFT and RPProof definitions agree (by `rfl`) |
| [`QFT.freeCovariance_reflection_positive_bilinear`](../OSforGFF/OS3_CovarianceRP.lean#L428) | $\mathrm{Re}\langle\Theta f, Cf\rangle \geq 0$ for complex $f \in \mathcal{S}_+$ |
| [`QFT.freeCovariance_reflection_positive_bilinear_real`](../OSforGFF/OS3_CovarianceRP.lean#L458) | $\langle\Theta f, Cf\rangle \geq 0$ for real $f \in \mathcal{S}_+$ |
| [`QFT.freeCovariance_reflection_positive_real`](../OSforGFF/OS3_CovarianceRP.lean#L479) | Alias for the real version |
...
| [`gaussianFreeField_real_generating_re`](../OSforGFF/OS3_GFF.lean#L329) | $\mathrm{Re}\  Z[h] = \exp(-\tfrac{1}{2}C(h,h))$ for real $h$ |
...
where $\omega = \sqrt{\|\vec{k}\|^2 + m^2}$. This yields the **mixed representation** (`bessel_bilinear_eq_mixed_representation`):
...
The inner integral involves $e^{-\omega|{-x_0 - y_0}|}$. For positive-time test functions ($x_0 > 0$, $y_0 > 0$), we have $|-x_0 - y_0| = x_0 + y_0$. This gives the key factorization:
...
The double $(x,y)$-integral then factors into $|F_\omega(\vec{k})|^2$ where $F_\omega$ is the **weighted Laplace-Fourier transform** (`weightedLaplaceFourier`):
...
Since $\omega > 0$ (because $m > 0$) and $|F_\omega|^2 \geq 0$, the integrand is pointwise non-negative, so the integral is non-negative. This is concluded in `freeCovariance_reflection_positive_direct`.
...
where $Z[f] = \exp(-\tfrac{1}{2}C(f,f)) > 0$.
